...
class Router {
    ...
    public function match(){ //метод проверки маршрута
        $url = trim($_SERVER['REQUEST_URI'] , '/'); //текущий адрес без "/" в начале (удаляет trim)
...
//                выполняем проверку на соответствие
    ...
    if($this->match()){ // выполнится если preg_match сработает и вернет true
        ...
        $path = 'application\controllers\\'. ucfirst($this->params['controller'].'Controller');//со сменой регистра аА
